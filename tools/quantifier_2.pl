...
## options
my %options=();
getopts("p:m:r:s:t:y:q:nxg:e:f:vjwT:PWU",\%options);
my $destin =$options{'q'};
...
print "$destin/$dir\n!!!!!!!!!!!!!!!\n";
my $outdir="${destin}/${dir}/${dir}_${time}";
if(not -d $outdir){
  mkdir($outdir);
}
print "$outdir\n!!!!!!!!!!!!!!!\n";
...
print $name0."!!!\n";
ConvertFastaFile($options{'p'},$name0,'precursor',"");
ConvertFastaFile($options{'m'},$name1,'mature',"");
...
if($organisms{$species} ){
   $command = "$Bin/make_html2.pl  -r $outdir -q $outdir/miRBase.mrd -k $name1$extension1 -t $organisms{$species} -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $t=`$command`;
     ...
     $command="$Bin/make_html2.pl -r $outdir -q $outdir/miRBase.mrd -k $name1$extension1 -t $species -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
    ...
    $command = "$Bin/make_html2.pl  -r $outdir -q $outdir/miRBase.mrd -k $name1$extension1 -y $time $opt_d $opt_o -i $outdir/${name1}_mapped.arf $starf $opt_l $opt_m -M miRNAs_expressed_all_samples_$time.csv $opt_P $opt_W";
...
#    print STDERR "\nbowtie -f -v 0 -a --best --strata --norc miRNA_precursor $name1.converted ${name1}_mapped.bwt\n\n";
	...
	if(!$options{'w'}){
$err = `bowtie -p $threads -f -v 0 -a --best --strata --al ${name1}_align_mature2precursor.fa --un ${name1}_unalign_mature2precursor --norc miRNA_precursor mature.converted ${name1}_mapped.bwt`;
    }
                
## map reads against precursors
    print STDERR "mapping read sequences against index\n";
    $err=`bowtie -p $threads -f -v $mismatches -a --best --strata --al ${name2}_align_reads2precursor.fa --un ${name2}_unalign_reads2precursor --norc miRNA_precursor  $name2.converted ${name2}_mapped.bwt`;
...
sub CreateOutputMRD{
	...
	$mature=`$Bin/convert_bowtie_output.pl ${name1}_mapped.bwt > ${name1}_mapped.arf`;
    ...
    if($options{'s'}){
        $star=`$Bin/convert_bowtie_outputf.pl ${name3}_mapped.bwt > ${name3}_mapped.arf`;
	...
	$reads=`$Bin/convert_bowtie_output.pl ${name2}_mapped.bwt > ${name2}_mapped.arf`;
