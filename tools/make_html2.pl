...
CloseHTML();
system("cp $outdir/expression_${time}.html expression_${time}.html");
...
sub PrintQuantifier{
    ...
    open HTML,">$outdir/expression_${time}.html" or die "cannot create expression_analyses/expression_analyses_${time}/expression_${time}.html\n";
    CreateHTML(); ##
    ...
    my %exprs;
	open IN,"<$outdir/miRNA_expressed.csv" or die "Error: File expression_analyses/expression_analyses_${time}/miRNA_expressed.csv not found\n";
...
#            die "$id\t$seq\n";
    ...
    open IN,"<$outdir/mature2hairpin" or die "Error: File expression_analyses/expression_analyses_${time}/mature2hairpin not found\n";
    my %hairpin2mature2;
	while(<IN>){
        chomp;
		my @line = split(/\t/);
