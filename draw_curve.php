            ...
            $line=fgets($fp);
             ...
             $line=fgets($fp);
                $arr1=explode("	",chop($line));
                    $genelength=count($arr)-1;
               foreach ($arr as $ss)
                {$result[($genelength-$ss+1)]=$arr1[$ss];
                if($ymax<$result[($genelength-$ss+1)]) $ymax=$result[($genelength-$ss+1)];
                }
   }
   fclose($fp);
   $result2=array();
   $region=array();
  $cut=$genelength-$getlength;   
   $i=0;
   ...
   {         $line=fgets($fp);
            ...
            {$line=fgets($fp);
               $arr=explode("	",chop($line));
                 ...
                 $result2[($genelength-$arr[2]+1)]=$arr[3];
                   
                 $region[($genelength-$arr[2]+1)]=$arr[1];
...
ImageFilledRectangle($im,$piclength-$left,$y-20+$down,$piclength,$y+$down,$gene_color);
 ImageString($im,2,$piclength-$left*0.7,$y-35+$down,$gene,$numColor);
 ImageFilledRectangle($im,$piclength-$left-100,$y+20,$piclength-$left-50,$y+40,$gene_color);
 ImageString($im,2,$piclength-$left-45,$y+20,"Target gene",$numColor);
  ImageFilledRectangle($im,$piclength-$left-100,$y+45,$piclength-$left-50,$y+65,$numColor);
 ImageString($im,2,$piclength-$left-45,$y+45,"Candidate Binding Regions",$numColor);
